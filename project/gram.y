...
assignment_expr
	: conditional_expr 
	| unary_expr assignment_operator assignment_expr {$$.tuple = tuple_assignment_expr( $1.tuple, $2.token, $3.tuple);}
	;
...
declarator2
	...
	| declarator2 '(' ')'
	| declarator2 '(' parameter_type_list ')' {$$.tuple = tuple_tail_to_head( $1.tuple, $3.tuple);}
	| declarator2 '(' parameter_identifier_list ')' {$$.tuple = tuple_tail_to_head( $1.tuple, $3.tuple);}
	;
...
parameter_list
	: parameter_declaration {$$.tuple = tuple_parameter_list( $1.tuple, 0);}
	| parameter_list ',' parameter_declaration {$$.tuple = tuple_parameter_list( $1.tuple, $3.tuple);}
	;
...
declaration_list
	: declaration
	| declaration_list declaration {$$.tuple = tuple_tail_to_head( $1.tuple, $2.tuple);}
	;

statement_list
	: statement
	| statement_list statement {$$.tuple = tuple_tail_to_head( $1.tuple, $2.tuple);}
	;
...
selection_statement
	: IF '(' expr ')' statement { $$.tuple = tuple_selection_statement( $3.tuple, $5.tuple, 0); } %prec LOWER_THAN_ELSE ;
	| IF '(' expr ')' statement ELSE statement { $$.tuple = tuple_selection_statement( $3.tuple, $5.tuple, $7.tuple); }
	| SWITCH '(' expr ')' statement { $$.tuple = tuple_selection_statement( $3.tuple, $5.tuple, 0); }
	;

iteration_statement
	: WHILE '(' expr ')' statement { $$.tuple = tuple_iteration_statement( $3.tuple, $5.tuple); }
...
file
	: external_definition
	| file external_definition { $$.tuple = tuple_tail_to_head( $1.tuple, $2.tuple); }
	;
...
function_definition
	: declarator function_body {$$.tuple = tuple_function_definition( 0, $1.tuple, $2.tuple);}
	| declaration_specifiers declarator function_body {$$.tuple = tuple_function_definition( $1.token, $2.tuple, $3.tuple);}
	;

function_body
	: compound_statement 
	| declaration_list compound_statement {$$.tuple = tuple_tail_to_head( $1.tuple, $2.tuple);}
	;
