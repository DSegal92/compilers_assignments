...
void	print_tuple( TUPLE *tuple)
{
	if( tuple)
	{
		printf( "next: %08.8x ", tuple->next);
		printf( "instr: %s ", instr_table[ tuple->token % I_SLEEP]);
		printf( "value: %02.2x ", tuple->value);
		printf( "address: %02.2x ", tuple->address);
		printf( "mask: %02.2x ", tuple->mask);
		if( tuple->mask)
		{
 ...
 */
	printf( "; automatic code generation for PIC16F84A\n");
	printf( "; EECS337 Compiler Design\n");
	printf( "; by: dls148, date: Fall 2012\n");
	printf( "; for PIC16F84A processor\n");
 ...
 */
	...
	printf( "	org	0x00	; on reset PC set to 0x00\n");
	printf( "reset:			\n");
	...
	printf( "	bsf	STATUS,RP0	; switch to bank 1 memory\n");
	printf( "	clrf	TRISA		; set PORTA to all outputs\n");
	printf( "	clrf	TRISB		; set PORTB to all outputs\n");
	printf( "	bcf	STATUS,RP0	; return to bank 0 memory\n");
/*
